...
function calculateBroadcastAddress(networkAddress, mask) {
    ...
    const networkNum = parseInt(networkAddress.replace(/\s/g, ''), 2);
    ...
    // Convert back to binary with spaces
    return broadcastNum.toString(2).padStart(32, '0').match(/.{8}/g).join(' ');
}
...
function calculateLastUsableIP(networkAddress, mask) {
    ...
    // Convert back to binary with spaces
    return lastUsableNum.toString(2).padStart(32, '0').match(/.{8}/g).join(' ');
}
